...
- 修复 AI 整章候选采纳后段落间距被重复放大的问题：常规双换行只作为段落边界，不再额外生成空段；明确的多空行仍保留为场景分隔。
- 修复整章中央审阅卸载正文编辑器后，“采纳并替换整章”因空 `editorRef` 静默退出的问题；整章候选现在可用保留的 React 正文基线完成版本校验、快照和统一写回，编辑器未就绪的其它模式会给出明确提示。
- 优化整章候选审阅：首次生成直接显示干净候选，不再把所有新增段落伪装成待逐项确认的 Diff；全量重写也默认阅读全文，Diff 改为按需开启并按连续差异片段跳转，整章采纳不要求逐项访问差异。
- 修复正文 AI 单次操作可能在上下文准备与请求收尾交界处形成并发任务、互相停止并让空结果覆盖已保留正文的问题；同一章节任务改为同步单飞，过期回调不得覆盖新任务，流式正文每秒写入可恢复缓存，中断后始终保留“从保留内容继续”入口。
- 修复 Windows 桌面端长篇正文流式生成约 120 秒后被固定总超时中断的问题；AI 流只保留建连超时和用户取消，中断前输出会持久保留，并可一键带入继续完成。
- 修复选区候选因 TipTap 纯文本与 HTML 分段表示不同而误报“正文版本冲突”；版本基线统一按编辑器 HTML 标准化计算，真实正文改动仍会安全阻止覆盖。
- 锁定选区新增“回到选区”入口，可将正文重新聚焦并滚动到选区中央；范围已失效时禁用定位和采纳。
- 正文 AI 完成防丢稿闭环：请求读取编辑器当前未保存正文，整章/续写/选区任务保存版本锚点；采纳前先校验、创建项目快照并经 `adopt()` 写回，异常完整回滚，成功后清会话并提供受版本保护的独立撤销。
- 正文上下文改为先解析本轮实际创作模型，再以同一 provider/model/窗口装配和发送；1M 窗口不再误按小窗口裁剪，章纲保持硬前置，细纲作为可选增强完整注入，并分别解释来源限额、窗口裁剪和语义压缩。
- 章节 AI 补齐生成/完成/停止/失败/阻断状态、选区持续高亮、讨论直接转改写、长选区反馈、危险清空确认、智能滚动、聊天气泡、面板拖拽与响应式抽屉；候选全文不再重复进入聊天。
- 新增选区字词级 diff 与整章中央稿纸审阅，显示字数/段落变化和大幅删改警告；通用候选守卫拦截非正文、Prompt/上下文回显和 Markdown，选区替换继承原富文本 marks 并保留场景空行。
- “提取长期事实”改为章节内可编辑、勾选、批量确认/放弃的候选工作区，证据逐字回查正文，确认写回统一经过 `FIELD_REGISTRY + AdoptionSchema + adopt()` 与事实账本状态机。
- Web 页面路由与首次迁移向导改为按需加载，恢复现有 bundle budget；Tauri dev/stable 最小窗口宽度降为 420px，为极窄正文/AI 独占布局提供真实桌面边界。
- 章节正文页统一 AI 协作入口：生成正文默认直接使用章纲与项目资料，作者可按需补充本次要求；未关联章纲时阻止盲写并引导补齐。无选区时默认修改整章，有选区时 AI 仍可读取整章上下文，但候选只能替换原选区，邻文回显会被拦截且所有正文改动均需用户确认采纳。
- 修复正文框选工具条闪现后消失的问题；选区操作改为进入同一 AI 协作对话继续迭代，不再与顶部快捷按钮、底部对话形成三套割裂流程。
- Windows 桌面安装包的最终交付位置统一为仓库根目录 `release/`，`src-tauri/target/**` 仅保留为编译中间目录，避免误把临时产物当成正式交付物。
- 长文本输入框达到高度上限后改为内部滚动，并阻止可滚动期间的滚轮事件被外层页面抢走。
- AI 上下文窗口设置增加自动保存状态、预设覆盖说明、格式校验和重置入口；逗号/空格分组可正常解析，非法输入不再清空已保存值。
- 本地模型与自定义 OpenAI-compatible 服务支持从 `/v1/models` 刷新模型列表并直接选择，失败时仍可手动输入。
- AI 配置预设改用各自独立的本机凭据槽；切换 A→B→A 或重启应用后会在 AI 配置界面恢复对应 Key，不再串用其它预设、供应商或端点的凭据；Gist 等其它 secret 不开放回显。
- JSON 备份新增细纲角色/场景角色/伏笔和创作规则参考的便携引用映射，新备份导入后不再保留旧项目 ID；旧备份继续兼容。
- 卷纲生成补齐全局骨架约束，通过正式上下文读取既有伏笔，并对齐主线、成长坐标、角色进退场和伏笔收放。
- AI 设置新增创作、提取、分析、审查四类任务模型路由，复用已有配置预设；未绑定、预设失效或缺少对应 API Key 时自动回退全局模型，消耗统计记录实际提供商、模型与任务类型。
- 正文页新增对照润色模式：左侧固定原稿、右侧独立富文本改写；保存前检查确定性物品连续性风险并创建项目快照，快照失败不会覆盖正文。
- 正文编辑器支持 `@` 搜索并插入角色、物品、地点和词条；已出现的实体名会显示非持久化档案标记，悬浮可查看摘要，且按当前世界隔离角色与词条。
- 提示词工作流中，用户手动修订过的步骤输出会作为后续步骤的正式输入，不再继续传递修改前的 AI 原始结果。
- CI 新增构建产物体积预算，分别守住应用入口、异步脚本、样式和 PDF worker，依赖意外进入首屏或分块异常膨胀时会直接报告超限文件。
- 源码 Release 增加发版前硬校验：版本 tag、`package.json` 和更新日志必须一致，并需完整质量闸门通过；手动发版不再允许旧 tag 指向另一提交。
- 数据管理新增本地诊断信息下载：只含应用/浏览器版本、数据库表记录数量和本次会话的脱敏错误位置，不读取作品内容、API Key 或 localStorage，也不会自动上传。
...
- **增强大纲章节拖拽**：大纲页支持把章节从一个卷拖到另一个卷，或从卷直挂拖进故事块；移动后源卷和目标卷的章节顺序会连续重排，正文仍绑定原章节节点。
- **新增全书查找替换**：章节页新增「查找替换」入口与 Ctrl/Cmd+F 快捷入口，支持单章/全书查找，单处/本章/全书替换，全字匹配、大小写敏感和正则；全书替换前会显示命中预览并自动创建项目快照，本次会话内可一键撤销。
- **大纲生成尊重已写正文进度**：补卷纲、整卷章纲和单章章纲时，会通过统一上下文源读取目标卷已写章节的章节记忆、结尾交接或正文短摘，并要求以已保存正文为事实边界。
- **统一版本号来源**：首页和侧边栏版本直接读取 `package.json`，避免 Release 已更新但界面仍显示旧版本。
- **修复题材包卷纲参数缺失**：所有系统卷纲模板统一提供「整体节奏 / 建议卷数」，并修正扩展玄幻、历史模板与卷纲生成器之间的变量契约。
- **修复 JSON 导入后工作区卡加载**：打开刚导入或直链进入的新项目时，项目会同步补入内存列表，不再需要刷新页面才能进入。
- **修复角色完整维度输入重复或丢失**：每个维度使用独立草稿，输入期间不接受父级旧值回灌；防抖、失焦和离开页面都会提交最新草稿，连续切换字段也不会漏保存。
- **修复角色关系编辑输入法异常**：关系标签与关系描述改用组合输入安全组件，避免 Windows 微软输入法 / 中文 IME 输入时拼音字母被受控输入重渲染打断并直接落入文本框。
- **修复角色关系保存反馈与项目串台风险**：新增、编辑和删除关系会显示保存结果；角色选择、AI 匹配和关系图仅使用当前项目数据，刷新后关系仍然存在。
- **补齐抽取关系反写角色词条**：采纳 AI 抽取关系后，经统一 `adopt()` 写回双方角色卡「人物关系」，保留手写内容并去重；手动新增的默认占位关系不会污染角色词条。
- **新增 CONSISTENCY-1：物品/状态账本硬校验**：基于 `itemLedger` 按规范章序投影“当前已持有物品”，新增 `heldItems` 上下文源注入正文生成，并在一致性审校中确定性提示“已持有物品被再次写成获得/首次获得”的风险。
- **修复物品重复获得类反馈**：章节生成和审校现在都能看到“截止当前章已持有物品”，减少前文刚获得的道具在后文被重新赋予获得途径的问题；本轮纯读，不改 IndexedDB schema、不迁移用户数据。
- **补齐物品栏编辑入口**：物品流水现在可在展开的时间线中直接修正物品名、数量、获得/消耗动作和备注，便于用户修正 AI 识别结果或手动维护道具状态。
- **大纲生成尊重已写正文进度**：补卷纲、整卷章纲和单章章纲时，会通过统一上下文源读取目标卷已写章节的章节记忆、结尾交接或正文短摘，并在提示词中要求以已保存正文为事实边界，不得重排或否认用户已写内容。
- **修复上一章上下文取错**：正文页审校摘要与情感节拍卡的“上一章结尾”改按大纲 canonical 章序查找，不再受章节正文记录创建顺序影响。
- **修复一键拆场景采纳异常**：细纲页和正文页内嵌场景面板都会把 AI 输出解析成结构化场景行，不再把整段输出塞进第一个场景备注。
- **放开本地 / 自定义 OpenAI-compatible 服务的空 API Key 误拦截**：Ollama、LM Studio 或无鉴权本地服务不再被导入、提取、参考分析等入口提前挡住；云端服务仍会由真实接口返回认证错误。
- **优化本地模型入口文案**：设置页将「Ollama (本地)」调整为「本地模型 (Ollama / LM Studio 等)」，提示 OpenAI-compatible `/v1` 地址和 Ollama/LM Studio 常见端口，减少本地模型用户走“自定义”的困惑。
- **新增 LongCat（美团）AI provider**：接入 OpenAI 兼容接口，补齐 provider 类型、模型预设、上下文窗口预算、设置页选项、代理路径与配置持久化回归测试。
- **修复世界观生成字段边界**：世界来源、力量体系、神明与信仰生成时增加字段边界提示，避免 AI 把上游来源、力量规则、神明设定互相改写。
- 数据安全：本组均为只读上下文 / 配置 / prompt 边界修复，不改 IndexedDB schema，不迁移用户数据。

## v3.7.5 — 2026-07-03 · 废弃 Windows 启动器 · 源码版 Release + npm 启动

> 本版是分发策略调整：仓库不再维护 `.bat`、`.command`、Windows `.exe` 或 Portable 启动器，避免用户继续被本地端口 / Service Worker / 内嵌静态服务器重定向问题阻塞。Release 保留 GitHub 自动生成的源码包，用户按 npm 文档启动。

- 删除仓库内启动器：`启动.bat`、`启动.command`、旧 Windows Portable / Go desktop server 打包线。
- Release 不再上传 exe / portable zip / SHA256 assets，仅保留 `Source code (zip)` 与 `Source code (tar.gz)`。
- README 与根目录文档统一指向 `使用npm指令启动项目.md`：下载源码 ZIP → 安装 Node.js LTS → `npm install` → `npm run dev`。
- 数据安全：本次只改分发入口和文档，不改 IndexedDB / localStorage / 项目数据结构。

## v3.7.4 — 2026-07-03 · Windows 内嵌服务重定向热修（历史过渡版）

- 修复旧 Windows 内嵌 Go 静态服务器在 `/storyforge/` base 路径下可能触发重定向循环的问题。
- 新增 npm 兜底启动说明，便于用户在启动器失效时自行打开项目。
- 说明：该路线随后已被 v3.7.5 废弃；后续不再维护 exe / portable 启动器。

## v3.7.3 — 2026-07-03 · 社区反馈批次部署 + 本地 PWA/SW 修复

- 部署 2026-07-01 社区反馈修复批次：场景细纲采纳崩溃、卷纲 / 章纲偏离主线、浅色主题可读性、伏笔边界说明与关联章节显示、角色驱动剧情上下文修复。
- 修复 CF-13：本地 `localhost / 127.0.0.1 / ::1` 禁用 PWA Service Worker 注册，并保留本地自愈清理逻辑；清理范围仅限 SW / Cache Storage，绝不触碰 IndexedDB / localStorage。
- 补发 Release，用于覆盖此前 Windows 启动重定向修复尝试。该分发路线后续已在 v3.7.5 改为源码 + npm。

## v3.7.2 — 2026-06-29 · 编辑器排版全局持久

- 正文编辑器排版偏好改为全局持久：字体、字号、行距、段距跨章节保持。
- 默认正文字号从 18 调整到 16，并修复选中文本调样式时工具栏/选区跳动问题。

## v3.7.1 — 2026-06-29 · 排版下拉交互修复

- 修复正文编辑器排版下拉点击闪烁、打不开的问题。

## v3.7.0 — 2026-06-29 · 长期一致性引擎 + Windows 打包线

- 长期一致性引擎上线：章节交接记忆、事实库、远距召回、一致性审计、正文修改影响分析等 NS-0～NS-6 能力进入主线。
- 该版本曾引入 Windows 打包线；后续因启动器问题在 v3.7.5 全面废弃，改为源码 Release + npm 启动。

## v3.7.3 — 2026-07-01 · 社区反馈修复批次（场景崩溃 / 主线 / 启动 / 主题 / 伏笔）

- **修复：灵感反推「第一遍不出来、要重推或退出重进才显示结果」**。根因是结果解析器只认 ```json``` 围栏或整段纯 JSON——模型带前后文（如「好的，以下是结果：{…}」）又没围栏时解析就失败、结果不显示。现改用健壮的 JSON 提取（围栏 / 未闭合围栏 / 裸括号起点 + 截断修复），第一遍就能稳定出结果。

- **修复：场景细纲「一键 AI 拆场景 → 采纳」后崩溃**（`scenes.reduce is not a function`）。根因是场景字段被当成 JSON 字符串入库，渲染时当数组用；现改为数组语义，且旧数据读取时自动回正、无需迁移、不动你已有内容。
- **修复：自动生成的卷纲 / 章纲偏离「故事设计」里填的主线**。卷纲、章纲生成现强制以故事主线为骨架：每卷 / 每章都要说明推进了主线的哪一段，不再另起主线。
- **修复：Windows 本地启动出现「127.0.0.1 重定向次数过多」**。本地打开时自动清理历史遗留的 service worker 缓存（只清缓存，绝不动你的作品和 API 配置）；`启动.bat` 曾增加端口占用检测。说明：该启动器路线后续已在 v3.7.5 废弃。
- **改进：Windows 小白启动引导（历史过渡）**。当时 README 增加过 Portable / `启动.bat` 引导；后续已被 v3.7.5 的源码包 + npm 启动说明取代。
- **改进：灵感反推使用边界更清楚**。输入区说明它适合短灵感 / 梗概 / 片段；长篇正文请改用「文档解析 / 项目参考导入」；输入过长时给非阻断提醒，不再静默只吃前半段。
- **改进：暖白 / 纸页 / 卷轴 / 冷灰等浅色主题可读性**。修正大量历史遗留的亮色文字 / 彩色徽标在浅底上发虚、看不清的问题（不影响熔炉深色主题）。
- **改进：伏笔功能更清楚、关联章节显示更准**。伏笔面板新增说明——伏笔不会自动改写已写正文，只在生成 / 续写对应章节时作为写作任务注入；修复伏笔「关联章节」显示与实际章节对不上的问题（原先误用大纲节点 ID 匹配）。
- **升级：伏笔写作任务上下文**。生成章节时，AI 会更清楚地收到本章的伏笔任务（该埋设 / 该呼应 / 该回收），并对「预期回收点已过」的伏笔给出逾期与临近提醒，用真实章节标题显示。

## v3.7.3 — 2026-06-30 · 角色设计全面升级：23 维度体系 + AI 补全 + 反向哺喂

- 角色维度从 8 项扩到 23 项网文向维度（身份职业·年龄性别种族·价值观信念·优点长处·缺点弱点·恐惧软肋·目标·核心矛盾·关键经历·实力境界·语言风格口头禅·习惯癖好·标志性物品等）：主要 / 次要 / NPC / 路人四个角色页都能看到、编辑完整维度（次要/NPC/路人默认收起、点开即全维度），统一收口到单一事实源 `CHARACTER_DIMENSIONS`，以后加一个维度全链路自动跟随。
- 「AI 设计角色」加维度勾选器：按戏份一键预设（主要全 / 次要 / NPC / 路人），自选这次要生成哪些维度；默认即产出全部维度（有内容才写、没有的留空，不硬凑编造）。
- 新增「AI 补全设定」按钮（每张角色卡上）：给已有角色补全缺失维度，默认勾选当前为空的维度，参考角色已有设定与世界观，只补选中字段、不覆盖其它——NPC 写着写着想升成主角时尤其顺手。
- 「AI 补全设定」内含「结合剧情已写内容（反向哺喂）」开关：把该角色在正文里已确认的事实 + 真实表现喂给 AI，补出来的维度贴合实际剧情、不与原文脱节。
- 生成章节正文时，角色上下文注入全部已填维度（此前只喂外貌/性格等老字段）：你设计的价值观 / 动机 / 弱点 / 目标真正影响 AI 写作。
- 全链路一致：AI 生成解析、采纳落库、灵感反推建角色、文档导入 / 合并重复角色、导出导入往返都走 23 维单源；文档解析仍只抽核心字段（省整本导入开销），导入后用「AI 补全」+「反向哺喂」按需丰富。
- 新增回归守卫 R-C1～R-C4：维度必登记必落库、补全只动选中字段不误伤既有、剧情证据召回按世界隔离、维度必进生成上下文。

## v3.7.0 — 2026-06-25 · Codex 审查补强：NS-4/NS-5 数据红线
...
## v3.7.0 — 2026-06-25 · NS-5 叙事感知混合检索（关键词 + embedding 语义通道）
...
## v3.7.0 — 2026-06-25 · NS-6 全闭环：影响分析 + 校验读事实/召回
...
## v3.7.0 — 2026-06-24 · NS-4 双层事实记忆（事实库）
...
## v3.7.0 — 2026-06-24 · 两个线上崩溃根治（根因·非补丁）
...
## v3.7.0 — 2026-06-18 · C 组设定库提取与下游产物重构
