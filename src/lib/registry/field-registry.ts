...
export const FIELD_REGISTRY: FieldSpec[] = [
  ...
  // NS-4 时序事实候选与状态机；章节侧栏只允许经 fact-ledger → adopt() 定点写回。
  num('temporalFacts', 'worldGroupId'),
  num('temporalFacts', 'characterId'),
  num('temporalFacts', 'locationId'),
  num('temporalFacts', 'storyArcId'),
  num('temporalFacts', 'subjectWorldGroupId'),
  num('temporalFacts', 'codexEntryId'),
  text('temporalFacts', 'subjectName', ['主体']),
  num('temporalFacts', 'objectCharacterId'),
  num('temporalFacts', 'objectLocationId'),
  num('temporalFacts', 'objectCodexEntryId'),
  text('temporalFacts', 'predicate', ['事实类型']),
  enumeration('temporalFacts', 'factKind', ['state', 'event', 'derived']),
  longtext('temporalFacts', 'value', ['事实值']),
  enumeration('temporalFacts', 'sourceType', ['chapter', 'manual', 'import', 'setting']),
  num('temporalFacts', 'sourceChapterId'),
  text('temporalFacts', 'sourceRecordTable'),
  num('temporalFacts', 'sourceRecordId'),
  longtext('temporalFacts', 'sourceQuote', ['证据引文']),
  num('temporalFacts', 'validFromChapterId'),
  num('temporalFacts', 'validToChapterId'),
  enumeration('temporalFacts', 'status', ['candidate', 'confirmed', 'rejected', 'superseded', 'stale', 'source-missing', 'invalid-range']),
  bool('temporalFacts', 'locked'),
  num('temporalFacts', 'confidence'),
  num('temporalFacts', 'supersedesFactId'),
