...
assert(routeSmoke.includes("const devIdentity = 'io.github.yuanbw2025.storyforge.dev'"), 'Desktop smoke 必须固定 dev identity')
assert(routeSmoke.includes('assertDevIdentityExecutable()'), 'Desktop smoke 必须在启动前拒绝 stable executable')
...
assert(upgradeSmoke.includes("run(process.execPath, [tauriCli, 'build', '--no-bundle', '--config', upgradeConfig])"), '覆盖升级 smoke 必须构建不同版本 artifact')
