...
| 规划日期 | 2026-07-14 |
| 技术路线 | 已由用户确认 |
| 当前状态 | M0～M2 PASS；下一里程碑 M3 · 作者真实数据切换 NOT STARTED |
...
| M1 · 合成数据全功能 | 生成唯一动作验收清单；完成窄 IPC、流式/取消、Credential Manager、不透明 secret 引用、受限文件/备份/外链和最小诊断；全部现有功能用合成数据验收 | PASS |
| M2 · 完整迁移演练 | 一个 `FullMigrationArchive` 纵向切片完成只读导出、空目标导入、hash/Blob/设置验证、重授权、激活、receipt 与回滚 | PASS |
...
- M1 已收口：生产 TypeScript AST 生成 1,351 个唯一 `actionId + FP ID` 并由 CI 防漂移；受限 Tauri IPC、Credential Manager dev/stable 命名空间、文件/备份、AI/Embedding/Gist、外链/剪贴板和脱敏诊断已落地；真实 dev identity smoke 覆盖 35 个可见模块、合成项目/章节重启、独立 profile、AI 流/取消、100 MiB Blob hash 和失败恢复。核心使用实跑 18.2 分钟/35 轮无失败后，作者明确豁免剩余时长；该豁免不等于自动跑满 30 分钟。当前状态与放行口径仍只看主蓝图 §17。
- M2 已收口：新增独立 `.storyforge-migrate` 格式和逐表迁移策略；旧网页入口只读导出，桌面首次启动只向空库导入，Rust 仅保存业务库外 journal/receipt，不接触业务表。合成 legacy 标记资料完成正常导入、2 MiB Blob、敏感配置剔除、失败清理、进程中断恢复和激活后回滚各一次；未反复跑相同场景，未启动浏览器、真实 profile 或默认正式 UDF。TypeScript、Web/dev/stable UI build、Rust 默认/全特性 test、fmt/clippy、确定性夹具与 1,361 项 parity 清单通过；独立 Claude 审查待进行。当前状态与放行口径仍只看主蓝图 §17。
...
- 最终标记 D1.1 PASS 前，D0 全部完成。
- 依据 2026-07-15 用户授权，可在 D0.2 / D0.4 / D0.5 闭环前并行实施开发隔离身份下的最小壳，但只能使用合成数据，不得接真实密钥、正式 profile 或真实用户数据，也不得据此宣称客户端可用。
- 不直接扩写 tmp/tauri-poc。
...
**当前状态（2026-07-15）**：`IN PROGRESS — dev identity 优先链路、同身份 exe 覆盖升级和启动顺序门已通过`；新构建 `3.7.5/3.7.6` dev artifacts 已通过合成项目、outline、章节正文 1.5 秒自动保存、正常关闭后的跨进程重启、独立空 profile、恢复原 profile、非密 localStorage 哨兵和同临时 exe 路径覆盖升级验证。运行态 IndexedDB version 370 / 42 stores，五条真实 Dexie upgrade fixtures 与迁移检查失败不执行 Prompt/Workflow seed 的顺序回归全绿；记录进程、动态端口和临时 profiles 均已清理，stable 默认 UDF 未打开或覆盖。卸载保留、stable 非 CDP 身份矩阵与失败注入尚未完成。详细进度见 [`windows-desktop/D1.3-PERSISTENCE-STATUS.md`](./windows-desktop/D1.3-PERSISTENCE-STATUS.md)。
...
公开发布路线（G2、D5、NSIS、受信任签名、Updater、beta/stable 和完整 Windows/卸载矩阵）等作者另行启动。M2 完整验证器和失败注入已完成；在作者明确启动 M3、确认正确源 profile 并单独授权前，仍禁止读取真实浏览器数据或向任何正式客户端 profile 导入数据。
