...
>
> **最后更新**: 2026-07-15（Windows Desktop 改按作者自用 M0～M3 收敛路线推进；当前 M0 IN PROGRESS，仍不可使用真实数据）
...
| 里程碑 | 当前状态 | 完成条件 |
|---|---|---|
| M0 · 稳定开发壳 | 🟠 IN PROGRESS；dev identity 的项目/outline/章节自动保存、重启、空 profile、恢复和同身份覆盖升级已通过 | 收口 stable 预启动拒绝调试、外部参数隔离、Cache/SW/UDF 隔离、pdf.js worker 证据和 AI manual 漂移 |
