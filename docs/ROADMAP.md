...
>
> **功能红线**：冻结生产版全部可达动作必须在 Windows 客户端保持等价。生产 `web-tab` 是唯一必需参考；Tauri + WebView2 必须逐项比较功能、规范化数据 hash、性能、恢复和安全。D0.4～D1 的 installed PWA 为可选补充，不阻塞主线；G1 仍要求 100% 登记且无 UNKNOWN/BLOCKED/兼容性 PARTIAL，G2 与 stable 仍要求动作级 manifest 100% PASS。
